...
def save_outputs(chunks: List[Chunk], verbose: bool = False, text_only: bool = False, output: str = 'outputs') -> None:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if chunk.image is not None:
            ...
            chunk.image.convert('RGB').save(f'{output}/{clean_path}_{i}.jpg')
            n_images += 1
    # Save the text
    with open(f'{output}/prompt.txt', 'w', encoding='utf-8') as file:
        file.write(text)
    if verbose: print_status(f"Output {len(text)/4} tokens and {n_images} images to '{output}'", status='success')
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--output', type=str, default='outputs', help='The output folder.')
...
if __name__ == '__main__':
    ...
    save_outputs(chunks=chunks, verbose=args.verbose, text_only=args.text_only, output=args.output)
